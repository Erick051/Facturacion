 ...
 function __construct() {
  ...
  public function consultar_comprobantes_pss() {
    ...
    if ( $numeroDeCampos < 1) { //Forza al usuario a utilizar al menos 3 campos para la busqueda
      ...
      // el administrador no tiene limites
      if ( $id_usuario != 1 ) {
    ...
    $url_consultar_comprobantes_pss = base_url()."index.php/mis_comprobantes_pss/consultar_comprobantes_pss";
    $data["url_consultar_comprobantes_pss"] = $url_consultar_comprobantes_pss;
    $this->db->order_by("fecha_timbrado DESC");
    $this->db->limit(300);
...
public function merge_masivo(){
    ...
    // se crea una carpeta para descargar los xmls
      ...
      foreach ($transacciones as $arr_trans) {
        ...
        if($xml != null){
          ...
          {
              $query2 = $this->db->query(str_replace('?', $arr_trans->id_trx33, $row_propiedad->valor_omision));
              $row_nombre = $query2->row();
          }
          
          // si se obtuvo nombre
          if ( $row_nombre != null ) {
              $filename_base = $row_nombre->nombre.".xml";
              $filename_base_pdf = $row_nombre->nombre.".pdf";
          } else {
              if ($xml->uuid == null ) {
                $filename_base = $id_trx33.".xml";
                $filename_base_pdf = $id_trx33.".pdf";
              } else {
                $filename_base = $xml->uuid.".xml";
                $filename_base_pdf = $xml->uuid.".pdf";
              }
          }
          ...
          fclose($file_1);
          $filename_for_zip_indiv = $filename_base;
        ...
        // si hay pdf
        if ( $pdf != null ) {
            $filename_for_zip_indiv = $filename_base_pdf;
      ...
      if ( $tipo_consulta == 1 ) {
          // se envia
          $filename = "RCIboveda_".date("YmdHis").".zip";
        ...
        $url_mis_cmprobantes = base_url()."index.php/mis_comprobantes_boveda/index";
          ...
          // descarga de zip
          $filename_for_zip = "RCIboveda_".date("YmdHis").".zip";
          $this->zip->download($filename_for_zip);
  ...
  public function anexos_descarga($id_anexo){
    ...
    $row_anexo = $arr_anexos_clientes->row();
    /*
  ...
  public function enviar_correo() {
    ...
    for ($i=0; $i < count($arr_mails); $i++) {
      ...
      $envio->fecha_proceso    = null;
      $envio->enviar_adjuntos  = 0;
      $envio->asunto           = "Ha recibido un Comprobante Fiscal Digital";
      
      $cuerpo_correo           = '<html>
<head></head>
<body>
	<table cellpadding="0" rule="none" >
		<tr>
			<td align="left" valign="top">
				<div><h3><b>Estimado Cliente.</b></h3>
				Se le informa que el d&iacute;a de hoy se generaron nuevas facturas que ya se encuentran en el Portal Web listas para su consulta y descarga en la siguiente liga:
				<br/>
				<br/>
				<center>
				<a href="http://www.rci-facturacionelectronica.com.mx">http://www.rci-facturacionelectronica.com.mx</a>
				</center>
				<br/>
				<br/>
				Cualquier duda o aclaraci&oacute;n favor de comunicarse a RCI al tel&eacute;fono (55) 5283 1000 opci&oacute;n 2 al departamento de Cr&eacute;dito y Cobranza o en el correo  <a href="atencionfacturacion@latam.rci.com">atencionfacturacion@latam.rci.com</a>
				<br/>
				<br/>
