...
class Facturacion_tickets extends CI_Controller {

	/**
	 * Funcionalidades para facturación de tickets y generación de factura global a partir de tickets no facturados
	 *
	 */
	public function index()
	{
        // este controlador no tiene por que entrar al index, ya que llega del ejecutor de facturacion, por tanto si entra al indice, se regresa a la pantalla principal
        $url_pantalla_principal = base_url()."index.php/pantalla_principal";
        redirect($url_pantalla_principal);
	...
	public function inicio_conciliacion_tickets_facturados()
	{        
        // se crea la variable para el paso de datos
        $data = array();
        $data["url_consultar_ano_mes"] = base_url()."index.php/facturacionTickets/facturacion_tickets/inicio_conciliacion_tickets_facturados";
        $data["url_conciliacion_busqueda"] = base_url()."index.php/facturacionTickets/facturacion_tickets/conciliacion_busqueda";
        $data["url_anterior"] = base_url()."index.php/facturacionTickets/facturacion_tickets/inicio_conciliacion_tickets_facturados";
        
        // si se recibio el ano y el mes de la consulta
        if ( $this->input->post('id_mes') != null ) {
            $mes = $this->input->post('id_mes');
        ...
        // se carga la GUI
        $this->cargar_interfaz_grafica($data, 'tickets/conciliacion/view_content_wrapper_inicio_conciliacion', 'tickets/conciliacion/view_script_inicio_conciliacion');
	}
    
	public function conciliacion_busqueda($p_mes, $p_ano, $p_organizacion, $p_sucursal)
	{
        ////$this->output->enable_profiler(TRUE);
        // se carga la interfaz para realizarl a conciliacion
        $data = array();
        $data["url_conciliacion_automatica"] = base_url()."index.php/facturacionTickets/facturacion_tickets/conciliacion_automatica/".$p_mes."/".$p_ano."/".$p_organizacion."/".$p_sucursal;
        $data["url_conciliacion_tickets_no_encontrados"] = base_url()."index.php/facturacionTickets/facturacion_tickets/conciliacion_tickets_no_encontrados/".$p_mes."/".$p_ano."/".$p_organizacion."/".$p_sucursal;
        $data["url_anterior"] = base_url()."index.php/facturacionTickets/facturacion_tickets/inicio_conciliacion_tickets_facturados";

        // se verifica si ya se tiene una conciliacion iniciada
        $this->db->where("mes",$p_mes);
        $this->db->where("ano",$p_ano);
        $this->db->where("id_organizacion",$p_organizacion);
        $this->db->where("id_sucursal",$p_sucursal);
        $arr_proceso_conciliacion = Model\Proceso_conciliacion_tickets::all();
        
        $id_proceso_conciliacion = 0;
        
        if ( count($arr_proceso_conciliacion) > 0 ) {
            foreach($arr_proceso_conciliacion as $proceso ) {
                $id_proceso_conciliacion = $proceso->id_proceso_conciliacion;
                break;
            }
        }
        
        if ( $id_proceso_conciliacion != 0 ) {
            // se obtiene la relacion de tickets conciliados automaticamente
            $this->db->where("id_proceso_conciliacion", $id_proceso_conciliacion);
            $tickets_preconciliados = Model\V_preconciliacion_automatica::all();
            $data["tickets_preconciliados"] = $tickets_preconciliados;
            
            // se obtiene la relacion de tickets conciliados manualmente
            $this->db->where("id_proceso_conciliacion", $id_proceso_conciliacion);
            $tickets_preconciliados_manual = Model\V_preconciliacion_manual::all();
            $data["tickets_preconciliados_manual"] = $tickets_preconciliados_manual;

        }
        
        // se obtienen las cifras de conciliacion
        $this->load->model("model_tickets_conciliacion");
        $arr_cifras_conciliacion = $this->model_tickets_conciliacion->obtener_cifras_conciliacion($p_sucursal, $p_mes, $p_ano);
        $data["arr_cifras_conciliacion"] = $arr_cifras_conciliacion;
        
        // se obtienen los datos de la sucursal
        $datos_sucursal = Model\Jos_hz_parties::find($p_sucursal);
        $data["datos_sucursal"] = $datos_sucursal;
        
        // se envian los parametros
        $data["p_mes"]          = $p_mes;
        $data["p_ano"]          = $p_ano;
        $data["p_organizacion"] = $p_organizacion;
        $data["p_sucursal"]     = $p_sucursal;
        $data["mes_en_texto"]   =  $this->mes_a_texto($p_mes);

        // se carga la GUI
        $this->cargar_interfaz_grafica($data, 'tickets/conciliacion/view_content_wrapper_conciliacion_busqueda', null);
	}
    
	public function conciliacion_automatica($p_mes, $p_ano, $p_organizacion, $p_sucursal)
	{
        ////$this->output->enable_profiler(TRUE);
        // se obtienen los datos del mapeo
        $data = array();
        $data["url_ejecutar_conciliacion_automatica"] = base_url()."index.php/facturacionTickets/facturacion_tickets/guardar_conciliacion_automatica";
        $data["url_anterior"] = base_url()."index.php/facturacionTickets/facturacion_tickets/conciliacion_busqueda/".$p_mes."/".$p_ano."/".$p_organizacion."/".$p_sucursal;
        
        $data["mes"]             = $p_mes;
        $data["ano"]             = $p_ano;
        $data["id_sucursal"]     = $p_sucursal;
        $data["id_organizacion"] = $p_organizacion;
        $data["id_usuario"]      = 1;
        ...
        // se ejecuta la consulta de conciliacion
        $this->load->model("model_tickets_conciliacion");
        $arr_facturas_manuales = $this->model_tickets_conciliacion->conciliacion_automatica(139, 7, 2016);
        $data["arr_facturas_manuales"] = $arr_facturas_manuales;


        // se carga la GUI
        $this->cargar_interfaz_grafica($data, 'tickets/conciliacion/view_content_wrapper_conciliacion_automatica',null);
        
	}
    
	public function guardar_conciliacion_automatica()
	{
        ////$this->output->enable_profiler(TRUE);
        // se guarda la url en la que se esta
        ...
        // numero de tickets conciliados
        $num_tickets_conciliados = $this->input->post('num_tickets_conciliados');
        
        // se verifica si ya existe el proceso
        ...
        if ( count($arr_proceso_conciliacion) < 1 ) {
            // se inserta
            // registro para proceso de conciliacion
            $proceso_conciliacion = new Model\Proceso_conciliacion_tickets();
            
            $proceso_conciliacion->id_proceso_conciliacion  = 0;
            $proceso_conciliacion->mes                      = $mes;
            $proceso_conciliacion->ano                      = $ano;
            $proceso_conciliacion->id_organizacion          = $id_organizacion;
            $proceso_conciliacion->id_sucursal              = $id_sucursal;
            $proceso_conciliacion->id_usuario               = $id_usuario;
            $proceso_conciliacion->fecha_inicio             = date("Ymd");
            $proceso_conciliacion->fecha_fin                = null;
            $proceso_conciliacion->id_estatus_conciliacion  = 2; // en proceso
            
            // se inserta el registro
            $proceso_conciliacion->save();
            
            // se obtiene el id de proceso de conciliacion
            $id_proceso_conciliacion = Model\Proceso_conciliacion_tickets::last_created()->id_proceso_conciliacion;
        } else {
            // se obtiene el id
            foreach ($arr_proceso_conciliacion as $proceso) {
                $id_proceso_conciliacion = $proceso->id_proceso_conciliacion;
        ...
        // se insertan los registros en la tabla de preconciliacion
        $this->db->where('id_proceso_conciliacion', $id_proceso_conciliacion);
        Model\Tickets_preconciliados::delete();
        for ($i = 1; $i <= $num_tickets_conciliados; $i++ ) {
            $ticket_preconciliado = new Model\Tickets_preconciliados();
            
            $indice_customer_trx_id_factura_manual = "ctid_factura_manual".$i;
            $indice_customer_trx_id_ticket         = "ctid_ticket".$i;

            $customer_trx_id_factura_manual = $this->input->post($indice_customer_trx_id_factura_manual);
            $customer_trx_id_ticket         = $this->input->post($indice_customer_trx_id_ticket);

            $ticket_preconciliado->id_ticket_preconciliado        = 0;
            $ticket_preconciliado->id_proceso_conciliacion        = $id_proceso_conciliacion;
            $ticket_preconciliado->id_origen_conciliacion         = 1; // conciliacion automatica
            $ticket_preconciliado->customer_trx_id_factura_manual = $customer_trx_id_factura_manual;
            $ticket_preconciliado->customer_trx_id_ticket         = $customer_trx_id_ticket;
            
            // se inserta el registro
            $ticket_preconciliado->save();
        }

        $conciliacion_busqueda = base_url()."index.php/facturacionTickets/facturacion_tickets/conciliacion_busqueda/".$mes."/".$ano."/".$id_organizacion."/".$id_sucursal;
        redirect($conciliacion_busqueda);
	}
    
	public function conciliacion_tickets_no_encontrados()
	{
        // se guarda la url en la que se esta
        //$url_actual = current_url();
        //$this->session->set_userdata("url_actual",$url_actual);
	...
	public function conciliacion_busqueda_tickets_no_encontrados()
	{
        // se guarda la url en la que se esta
        //$url_actual = current_url();
        //$this->session->set_userdata("url_actual",$url_actual);
	...
	public function vista_previa_conciliacion()
	{
        // se guarda la url en la que se esta
        //$url_actual = current_url();
        //$this->session->set_userdata("url_actual",$url_actual);
	...
	public function ejecutar_conciliacion()
	{
        // se guarda la url en la que se esta
        //$url_actual = current_url();
        //$this->session->set_userdata("url_actual",$url_actual);
	...
	public function inicio_conciliacion()
	{
        // se guarda la url en la que se esta
        //$url_actual = current_url();
        //$this->session->set_userdata("url_actual",$url_actual);
    ...
    public function cargar_interfaz_grafica($data, $cuerpo, $script) {
        ...
        $this->load->view('view_main_footer_pantalla_principal');
        $this->load->view('view_aside_control_rightbar_pantalla_principal');
        $this->load->view('view_wrapper_fin_pantalla_principal');
        $this->load->view('view_script_principal');
    ...
    public function factura_global(){
        $this->load->view('view_html_head_principal');
        $this->load->view('view_body_pantalla_principal');
        $this->load->view('view_mainheader_pantalla_principal');
        $this->load->view('view_main_aside_left_bar_pantalla_principal');
        $this->load->view('tickets/factura_global/view_factura_global'); // vista que contiene el cuerpo
        $this->load->view('view_main_footer_pantalla_principal');
        $this->load->view('view_aside_control_rightbar_pantalla_principal');
        $this->load->view('view_script_principal');
        $this->load->view('tickets/factura_global/view_script_config_factura_global');
        $this->load->view('view_body_html_cierre');
    }
    public function insert_factura_global(){
        $this->load->model("model_factura_global");
        $periodo= $this->input->post("periodo");
        $diarioh= $this->input->post("diarioh");
        $descripciond= $this->input->post("descripcion");
        ...
        if ($this->model_factura_global->insert_factura_global($periodo, $diarioh, $descripcion, $si)==true){
            $true['si']="true";
            $this->load->view('view_html_head_principal');
            $this->load->view('view_body_pantalla_principal');
            $this->load->view('view_mainheader_pantalla_principal');
            $this->load->view('view_main_aside_left_bar_pantalla_principal');
            $this->load->view('/tickets/factura_global/view_factura_global', $true); // vista que contiene el cuerpo
            $this->load->view('view_main_footer_pantalla_principal');
            $this->load->view('view_aside_control_rightbar_pantalla_principal');
            $this->load->view('view_script_principal');
    ...
    // cambiar mes numerico a mes en texto
    public function mes_a_texto($p_mes) {
        $mes_en_texto = "Enero";
        switch($p_mes) {
            case 1:
              $mes_en_texto = "Enero";
            ...
            case 1:
              $mes_en_texto = "Septiembre";
              break;

            case 1:
              $mes_en_texto = "Octubre";
              break;

            case 1:
              $mes_en_texto = "Noviembre";
              break;

            case 12:
              $mes_en_texto = "Diciembre";
              break;              
        }
